...
async function plannerLogin() {
  // перечитываем .env: пароль мог смениться через UI «Настройки»
  const fs = require('fs');
  ...
  try {
    ...
    for (const line of fs.readFileSync(envPath, 'utf8').split('\n')) {
      ...
      if (m && !process.env[m[1] + '_LOCKED']) process.env[m[1]] = m[2];
    }
  } catch {}
...
// --- Контекст дня для ИИ: активные + недавно выполненные задачи ---
async function tasksContext() {
  ...
  const done = await pool.query(
    `SELECT id, title FROM task WHERE status='DONE' ORDER BY completed_at DESC LIMIT 15`);
  const active = rows.map(r => `#${r.id} [${r.due_date || 'без срока'}${r.due_time ? ' ' + r.due_time.slice(0,5) : ''}] ${r.title}${r.project_name ? ' (' + r.project_name + ')' : ''}`).join('\n') || '(нет)';
  const doneList = done.rows.map(r => `#${r.id} ${r.title}`).join('\n') || '(нет)';
  return `Активные задачи:\n${active}\n\nНедавно выполненные (можно удалить: delete):\n${doneList}`;
}
...
Тебе доступны действия над задачами из списков в контексте (у каждой id: #N):
- создать новую задачу (tasks)
- отметить выполненной (complete: [id])
- удалить — активную или выполненную (delete: [id])
- перенести на дату (move: [{id, due_date}])
Просьбы вида «удали все выполненные тестовые» → собери их id в delete.
